...
| Docker 构建与 SSH 发布 | Moby Client + `golang.org/x/crypto/ssh` | Moby 官方客户端连接隔离的 Docker-in-Docker 构建节点；Go 官方扩展库负责目标主机密码/私钥认证和镜像流传输 | 表单保存前实际执行 `docker version` 并固定 SHA256 主机指纹；有仓库时受控 push/pull，无仓库时把 `docker save` 流传给 SSH 主机执行 `docker load` 并校验镜像 ID，不开放宿主机终端 |
