# EDO 外部 Git Webhook API

EDO 可以接收普通 Git、GitHub、GitLab、Gitea 和 Gitee 的代码事件。此入口默认关闭，管理员需要先在“系统设置”中开启“Git Webhook API”，再在具体代码仓库中启用 Webhook。
...
4. 在 Git 平台配置公网可访问的 HTTPS 地址。完整地址由 EDO 的外部访问域名和以下路径组成：
...
| 普通 Git | `X-EDO-Event` | `X-EDO-Delivery` | `X-EDO-Signature-256: sha256=<HMAC-SHA256>` |

HMAC-SHA256 必须针对收到的原始请求体计算，不能先解析再重新序列化。投递 ID 用于去重；普通 Git 未提供投递 ID 时，EDO 使用请求体摘要作为稳定 ID。
...
X-EDO-Event: push
X-EDO-Delivery: 8da1c56f-0a54-4b88-8bd5-b4a978302c89
X-EDO-Signature-256: sha256=<hex digest>
...
Tag Push 只需把 `ref` 改为 `refs/tags/v1.4.0`。删除分支或 Tag 时平台通常把提交版本设为全零，EDO 会保留事件但不把全零值当作可发布版本。
...
POST /api/v1/webhooks/git/{repository_id}
Content-Type: application/json
X-EDO-Event: pull_request
X-EDO-Delivery: 4cd49e26-8e60-41e4-8cc0-19ae0c43fe43
X-EDO-Signature-256: sha256=<hex digest>
