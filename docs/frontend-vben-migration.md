...
旧 React 运行时、TSX 页面、React Router、Zustand 和旧样式入口已删除。浏览器中已有登录会话仍然有效；旧页面 URL 通过 Vue Router 重定向到新的页面结构。历史主题键 `zrt.theme` 继续复用，用户不需要重新选择主题。旧流水线图、节点定义和运行快照不迁移到新编辑器。
