...
根目录 `.env` 同时保存运行连接。直接运行二进制或 Mage 时使用 `ZRT_DATABASE_*`、`ZRT_REDIS_URL` 和 `ZRT_NATS_URL`；Compose 容器使用 `.env` 中的 `ZRT_COMPOSE_DATABASE_*`、`ZRT_COMPOSE_REDIS_URL` 和 `ZRT_COMPOSE_NATS_URL`，再映射成容器进程实际读取的变量。两组变量必须分开，因为宿主机的 `127.0.0.1`、相对 SQLite 路径不能在容器内原样使用。
...
Compose 默认使用持久化 SQLite，适合单机。需要横向扩容 ZRT 实例时，将 `.env` 中的 `ZRT_COMPOSE_DATABASE_DRIVER` 和 `ZRT_COMPOSE_DATABASE_DSN` 改为 PostgreSQL 或 MySQL，并把 `ZRT_COMPOSE_REDIS_URL`、`ZRT_COMPOSE_NATS_URL` 指向高可用 Redis 与 NATS JetStream。连接 URL 可以携带认证信息，但 `.env` 不得提交且应限制为仅部署账户可读。每个实例都是包含 API 和后台任务 Goroutine 的完整单进程服务。
