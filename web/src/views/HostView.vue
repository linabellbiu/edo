    ...
    <div class="host-layout vben-card">
      ...
      <main v-if="selected" class="host-detail">
        ...
        <section class="capability-section">
          <header><div><h4>{{ selected.is_builtin ? '本地的主机能力' : '主机能力' }}</h4><p>{{ selected.is_builtin ? '本地 Docker 与直接终端执行能力由当前 EDO 运行环境决定。' : 'SSH 用于执行部署方案脚本；Docker 与 Kubernetes 的运行资源在“运行资源”中操作。' }}</p></div></header>
