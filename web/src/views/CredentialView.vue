...
function reset() { Object.assign(form,{name:'',provider:'github',auth_type:'token',username:'',secret:''}); editingID.value=''; formOpen.value=false }
...
async function submit(){submitting.value=true;try{const payload={...form,secret:form.secret||null};if(editingID.value)await client.put(`/git-credentials/${editingID.value}`,payload);else await client.post('/git-credentials',payload);message.success('Git 凭据已保存');reset();await refresh()}catch(error){message.error(apiErrorMessage(error))}finally{submitting.value=false}}
  ...
  <section>
    ...
    <a-drawer v-model:open="formOpen" :title="editingID ? '修改 Git 凭据' : '保存 Git 凭据'" width="520" @close="reset">
      ...
      <a-form layout="vertical" @finish="submit">
        ...
        <a-form-item :label="form.auth_type === 'token' ? '令牌' : 'SSH 私钥'" :required="!editingID"><a-textarea v-model:value="form.secret" :rows="form.auth_type === 'ssh_key' ? 8 : 3" :placeholder="editingID ? '留空表示保持原值' : '请输入凭据内容'" /></a-form-item>
        <a-button type="primary" html-type="submit" block :loading="submitting"><KeyRound :size="15" />保存</a-button>
      </a-form>
    </a-drawer>
  </section>
</template>
