...
const canExecuteQueueAction = computed(() => auth.canAny(['monitor.execute']))
...
<template>
  <section>
    <PageToolbar description="直接查看 EDO 节点、进程、Worker、任务队列与数据库连接池，页面每 5 秒自动刷新。"><span v-if="metrics" class="metrics-time">{{ new Date(metrics.collected_at).toLocaleTimeString('zh-CN',{hour12:false}) }}</span><a-button :loading="loading" @click="refresh"><RefreshCw :size="15" />立即刷新</a-button></PageToolbar>
    ...
    <a-spin :spinning="loading && !metrics"><template v-if="metrics">
      <div class="metric-cards">
        <article class="metric-card vben-card"><div><Cpu /><span>节点 CPU</span></div><strong>{{ metrics.host.cpu_percent.toFixed(1) }}%</strong><p>{{ metrics.host.logical_cpus }} 个逻辑核心 · EDO {{ metrics.process.cpu_percent.toFixed(1) }}%</p><a-progress :percent="metrics.host.cpu_percent" :show-info="false" /></article>
...
<style scoped>
.metrics-time{color:var(--edo-muted);font-size:12px}.metric-alert{margin-bottom:14px}.metric-cards{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:14px}.metric-card{padding:18px}.metric-card>div{display:flex;align-items:center;gap:8px;color:var(--edo-muted)}.metric-card svg{width:19px;color:var(--edo-primary)}.metric-card strong{display:block;margin:18px 0 5px;font-size:25px}.metric-card p{height:20px;margin:0 0 15px;color:var(--edo-muted);font-size:12px}.monitor-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px;margin-top:14px}.monitor-panel{padding:19px}.monitor-panel header{display:flex;align-items:center;justify-content:space-between}.monitor-panel header span{color:var(--edo-muted);font-size:11px}.monitor-panel h3{margin:2px 0 0}.monitor-panel header>svg{width:21px;color:var(--edo-primary)}.queue-actions{display:flex;align-items:center;gap:8px}.queue-actions .ant-tag{margin-inline-end:0}dl{display:grid;grid-template-columns:repeat(3,1fr);gap:1px;margin:17px 0 0;background:var(--edo-border)}dl>div{padding:13px;background:var(--edo-surface)}dt{color:var(--edo-muted);font-size:11px}dd{margin:5px 0 0;font-weight:650}.danger{color:#e94f5f}@media(max-width:1150px){.metric-cards{grid-template-columns:repeat(2,1fr)}}@media(max-width:760px){.metric-cards,.monitor-grid{grid-template-columns:1fr}dl{grid-template-columns:repeat(2,1fr)}}
</style>
