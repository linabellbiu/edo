...
function buildPlanRegistry(plan: BuildPlan) {
  if (plan.kind === 'script') return 'ZRT 文件制品存储'
...
function artifactStorageLabel(kind: Artifact['storage_kind']) {
  return ({ local_file: 'ZRT 制品存储', registry: '镜像仓库', docker_daemon: '构建运行时' } as const)[kind] || kind
}
      ...
      <div class="build-plan-tree-scroll">
        <div v-for="plan in plans" :key="plan.id" class="build-plan-tree-item" :class="{ selected: selectedID === plan.id }">
          <button type="button" class="build-plan-select" @click="selectPlan(plan.id)">
            ...
            <span class="build-plan-copy"><strong>{{ plan.name }}</strong><small>{{ buildPlanKindLabel(plan.kind) }} · {{ buildPlanArtifact(plan) }}</small></span>
    ...
    <main v-if="selected" class="build-plan-detail">
      <header class="build-plan-detail-head">
        ...
        <div>
          ...
          <a-button v-if="canManage" :loading="mutationID === selected.id" @click="emit('toggle', selected)">{{ selected.is_active ? '停用' : '启用' }}</a-button>
          <a-button v-if="canManage" @click="emit('edit', selected)">编辑</a-button>
          <a-button v-if="canManage" danger :disabled="mutationID === selected.id" @click="emit('remove', selected)">删除</a-button>
        </div>
      </header>
      ...
      <section v-else class="build-plan-artifacts">
        <header class="artifact-toolbar">
          ...
          <div>
            ...
            <a-upload v-if="canManage" :show-upload-list="false" :before-upload="beforeArtifactUpload" :disabled="uploadDisabled || artifactUploading">
        ...
        <a-alert
          ...
          type="info"
          show-icon
          :message="selectedApplication ? `当前层级：${selected.name} / ${selectedApplication.name}` : `当前层级：${selected.name} / 全部应用`"
...
<style scoped>
.build-plan-layout{display:grid;min-height:570px;grid-template-columns:286px minmax(0,1fr);overflow:hidden}.build-plan-tree{border-right:1px solid var(--zrt-border);background:var(--zrt-surface-soft)}.build-plan-tree>header{display:flex;height:64px;align-items:center;justify-content:space-between;padding:0 16px}.build-plan-tree>header strong,.build-plan-tree>header small{display:block}.build-plan-tree>header small{margin-top:2px;color:var(--zrt-muted);font-size:11px}.build-plan-tree>header>span{min-width:28px;padding:3px 8px;border-radius:999px;color:var(--zrt-muted);background:var(--zrt-surface);text-align:center}.build-plan-tree-scroll{max-height:calc(100vh - 230px);overflow-y:auto;padding:0 7px 10px}.build-plan-tree-item{margin:3px 0;border-radius:11px}.build-plan-tree-item.selected{background:var(--zrt-surface)}.build-plan-select{display:grid;width:100%;min-height:68px;align-items:center;grid-template-columns:38px minmax(0,1fr) 8px;gap:10px;padding:9px 10px;border:0;border-radius:10px;color:var(--zrt-text);background:transparent;cursor:pointer;text-align:left}.build-plan-select:hover{background:color-mix(in srgb,var(--zrt-primary) 5%,var(--zrt-surface))}.build-plan-tree-item.selected>.build-plan-select{box-shadow:inset 3px 0 var(--zrt-primary)}.build-plan-icon{display:grid;width:38px;height:38px;place-items:center;border-radius:11px;color:#4f72f2;background:var(--zrt-primary-soft)}.build-plan-icon.script{color:#9b6fe8;background:color-mix(in srgb,#9b6fe8 10%,var(--zrt-surface))}.build-plan-icon svg{width:19px}.build-plan-copy{min-width:0}.build-plan-copy strong,.build-plan-copy small{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.build-plan-copy small{margin-top:3px;color:var(--zrt-muted);font-size:11px}.build-plan-select>i{width:8px;height:8px;border-radius:50%;background:#28b66e}.build-plan-select>i.inactive{background:#a8adb7}.build-plan-children{position:relative;display:grid;gap:2px;margin:0 8px 7px 27px;padding:3px 0 3px 17px}.build-plan-children::before{position:absolute;top:0;bottom:0;left:5px;width:1px;background:var(--zrt-border);content:""}.build-plan-children>button{position:relative;display:flex;height:34px;align-items:center;gap:8px;padding:0 10px;border:0;border-radius:8px;color:var(--zrt-muted);background:transparent;cursor:pointer;text-align:left}.build-plan-children>button::before{position:absolute;top:50%;left:-12px;width:12px;height:1px;background:var(--zrt-border);content:""}.build-plan-children>button:hover,.build-plan-children>button.active{color:var(--zrt-primary);background:var(--zrt-primary-soft)}.build-plan-children svg{width:15px}.build-plan-detail{min-width:0;padding:22px}.build-plan-detail-head{display:flex;align-items:flex-start;justify-content:space-between;gap:20px;padding-bottom:18px;border-bottom:1px solid var(--zrt-border)}.build-plan-detail-head>div:first-child{min-width:0}.build-plan-detail-head span,.build-plan-detail-head p{color:var(--zrt-muted)}.build-plan-detail-head h3{overflow:hidden;margin:3px 0 2px;font-size:21px;text-overflow:ellipsis;white-space:nowrap}.build-plan-detail-head p{overflow:hidden;margin:0;text-overflow:ellipsis;white-space:nowrap}.build-plan-detail-head>div:last-child{display:flex;flex:0 0 auto;align-items:center;gap:7px}.build-plan-overview{padding-top:20px}.build-plan-summary{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:11px}.build-plan-summary article{display:flex;min-width:0;align-items:center;gap:12px;padding:15px;border:1px solid var(--zrt-border);border-radius:11px;background:var(--zrt-surface-soft)}.build-plan-summary article>span{display:grid;width:38px;height:38px;flex:0 0 38px;place-items:center;border-radius:10px;color:var(--zrt-primary);background:var(--zrt-surface)}.build-plan-summary svg{width:19px}.build-plan-summary article>div{min-width:0}.build-plan-summary small,.build-plan-summary strong{display:block}.build-plan-summary small{color:var(--zrt-muted);font-size:11px}.build-plan-summary strong{overflow:hidden;margin-top:3px;text-overflow:ellipsis;white-space:nowrap}.build-plan-limit{display:flex;align-items:center;gap:12px;margin-top:12px;padding:14px 16px;border-radius:10px;background:var(--zrt-primary-soft)}.build-plan-limit>svg{width:20px;color:var(--zrt-primary)}.build-plan-limit small,.build-plan-limit strong{display:block}.build-plan-limit small{color:var(--zrt-muted);font-size:11px}.build-plan-limit p{margin:0 0 0 auto;color:var(--zrt-muted);font-size:12px}.build-plan-artifacts{padding-top:18px}.artifact-toolbar{display:flex;align-items:center;justify-content:space-between;gap:16px;margin-bottom:12px}.artifact-toolbar>div:first-child strong,.artifact-toolbar>div:first-child small{display:block}.artifact-toolbar>div:first-child small{margin-top:2px;color:var(--zrt-muted);font-size:11px}.artifact-toolbar>div:last-child{display:flex;flex:0 0 auto;align-items:center;gap:8px}.artifact-toolbar :deep(.ant-select){width:210px}.artifact-toolbar :deep(.ant-btn){display:inline-flex;align-items:center;gap:5px}.artifact-toolbar svg,.table-actions svg{width:14px}.artifact-context{margin-bottom:12px}.artifact-digest{white-space:nowrap}.build-plan-empty{display:grid;place-items:center}
@media(max-width:900px){.build-plan-layout{grid-template-columns:1fr}.build-plan-tree{max-height:310px;border-right:0;border-bottom:1px solid var(--zrt-border)}.build-plan-tree-scroll{max-height:240px}.build-plan-detail{padding:17px}}
