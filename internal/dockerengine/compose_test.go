...
func TestRunComposeCLIStreamsInlineConfigurationWithFixedArguments(t *testing.T) {
	...
	input := ComposeDeployInput{
		...
		Image: "zrt.local/api:fixed", Timeout: 45 * time.Second,
	}
	endpoint := &model.DockerEndpoint{ID: "unix-endpoint", Host: "unix:///var/run/docker.sock", IsActive: true}
	if err := service.runComposeCLI(context.Background(), endpoint, "zrt-project", input); err != nil {
	...
	wantArguments := strings.Join(composeCommandArguments("", "zrt-project", "api", 45*time.Second), "\n")
	...
	if !strings.Contains(values, "DOCKER_HOST=unix:///var/run/docker.sock\n") ||
		!strings.Contains(values, "ZRT_IMAGE=zrt.local/api:fixed\n") ||
...
func TestRunComposeWithSSHUsesFixedCommandAndStreamsYAMLAfterSudoPassword(t *testing.T) {
	...
	if err := runComposeWithSSH(context.Background(), connector, bundle, "zrt-project", input); err != nil {
	...
	command = <-commands
	if !strings.HasPrefix(command, "sudo -S -p '' -- env ZRT_IMAGE='zrt.local/api:fixed' docker 'compose'") ||
...
func TestRunComposeCLIReportsMissingPlugin(t *testing.T) {
	...
	}, "zrt-project", ComposeDeployInput{
		ServiceName: "api", YAML: validComposeYAML, Image: "zrt.local/api:fixed", Timeout: 45 * time.Second,
	})
...
func TestRunComposeWithSSHReportsMissingPlugin(t *testing.T) {
	...
	err = runComposeWithSSH(context.Background(), connector, bundle, "zrt-project", ComposeDeployInput{
		ServiceName: "api", YAML: validComposeYAML, Image: "zrt.local/api:fixed", Timeout: 45 * time.Second,
	})
	if !errors.Is(err, ErrComposePluginUnavailable) {
...
func TestComposeProjectNameDoesNotExposeTargetIdentifier(t *testing.T) {
	...
	if first != second || !strings.HasPrefix(first, "zrt-") || strings.Contains(first, "customer") {
